...
record_link(dladm_handle_t dh, datalink_id_t linkid, void *arg) {
	...
	if (bucket->len == bucket->sz) {
		...
		if (rnic == NULL) {
			PROM_WARN("Unable to allocate NIC buffer. Link %d ignored.", linkid);
		...
		for (k = 0; k < nb->len; k++) {
			...
			if (nb->nic[k].zid != 0) {
				if (getzonenamebyid(nb->nic[k].zid, zname, ZONENAME_MAX) == -1) {
					...
					sprintf(zname, "link%d", nb->nic[k].lid);
	...
	bool compact, hrtime_t now)
{
	...
	if (!compact)
		addPromInfo4(SOLMEX_NET_NAME_PREFIX, snames[NET_IDX_IFSPEED_BPS],
	...
	nic_stat_quantity_t ntype, nic_filter_chain_t *nfc)
{
	...
	for (m = 0; m < stats_sz; m++) {
		net_idx_t l = stats[m];
		if (!compact) {
			addPromInfo4(SOLMEX_NET_NAME_PREFIX, snames[l],
