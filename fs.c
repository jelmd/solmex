	...
	while (*t) {
		...
		if (c != NULL) {
			...
			if (zchain == NULL) {
				...
				if (zchain == NULL) {
					fprintf(stderr, "ERROR (fsops): Unable to create zinfo: %s\n", strerror(errno));
					ok = false;
					break;
				}
				zcount++;
				...
				if (zchain->zid == -1)
					fprintf(stderr, "WARNING (fsops): Zone '%s' not found!\n", t);
		...
		if (!found) {
			fprintf(stderr, "WARNING (fsops): Unknown fstype '%s' for %s zone ignored",
...
collect_fs(psb_t *sb, bool compact, kstat_ctl_t *kc, hrtime_t now, void *cfg) {
	...
	if (revalidate) {
		...
		if (((zinfo_t *) cfg)->next)
			((zinfo_t *) cfg)->next = sortZinfos(last_cfg->next);	// odd
	}
